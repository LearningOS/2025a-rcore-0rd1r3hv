...
pub fn sys_fstat(fd: usize, st: *mut Stat) -> isize {
    ...
    let task = current_task().unwrap();
    if let Some(file) = task.fd_to_file(fd) {
        if let Some(st_tmp) = file.fstat() {
            let st_tmp_byte_ptr = &st_tmp as *const Stat as *const u8;
            let mut st_tmp_slice = unsafe { from_raw_parts(st_tmp_byte_ptr, size_of::<Stat>()) };
            let buffers =
                translated_byte_buffer(current_user_token(), st as *const u8, size_of::<Stat>());
            for buffer in buffers {
                buffer.copy_from_slice(&st_tmp_slice[..buffer.len()]);
                st_tmp_slice = &st_tmp_slice[buffer.len()..];
            }
            0
        } else {
            -1
