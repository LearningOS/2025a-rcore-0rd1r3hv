...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let mut ts_tmp_slice = unsafe { from_raw_parts(ts_tmp_byte_ptr, size_of::<TimeVal>()) };
    for buffer in buffers {
        buffer.copy_from_slice(&ts_tmp_slice[..buffer.len()]);
        ts_tmp_slice = &ts_tmp_slice[buffer.len()..];
    }
    0
}
